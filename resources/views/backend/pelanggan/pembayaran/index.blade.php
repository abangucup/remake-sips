...
<div class="row">
    ...
    @if ($pelanggan->pembayarans->isEmpty() || ($tagihan &&
    ...
    @endif
    {{-- <div class="col-xl-{{ $pelanggan->pembayarans->isEmpty() || ($tagihan && \Carbon\Carbon::parse($tagihan->tanggal_bayar)->isoFormat('MMMM') !==
        \Carbon\Carbon::parse(now())->isoFormat('MMMM') ? '12' : '12') }}"> --}}
        <div class="col-xl-{{ ($tagihan && \Carbon\Carbon::parse($tagihan->tanggal_bayar)->isoFormat('MMMM') !==
            \Carbon\Carbon::parse(now())->isoFormat('MMMM')) || $pelanggan->pembayarans->isEmpty() ? '8' : '12' }}">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">Pembayaran Saya</h4>
                </div>
                        ...
                        <table id="example" class="display" style="width: 100%">
                            ...
                            <tbody>
                                ...
                                <tr class="text-center">
                                    ...
                                    <td>{{ Carbon\Carbon::parse($pembayaran->tanggal_bayar)->isoFormat('LLLL') }}</td>
                                    ...
                                    {{-- <td>
                                        <button type="button" data-bs-toggle="modal" {{ $pembayaran->status_bayar ==
                                            'lunas' ? 'disabled' : '' }}
